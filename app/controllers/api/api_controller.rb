class Api::ApiController < ApplicationController
  ...
  def process_token
    if request.headers['Authorization'].present?
      begin
        jwt_payload = JWT.decode(request.headers['Authorization'].split(' ')[1], Rails.application.credentials.dig(:secret_key_base)).first
        @current_user_id = jwt_payload['user'][0]['id']
